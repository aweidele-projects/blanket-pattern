        ...
        <div className="h-full w-full overflow-auto absolute top-0 left-0">
          <ul className="grid grid-cols-3 sm:grid-cols-2 gap-3 w-full">
            {Object.entries(currentColors).map(([key, yarn]) => (
              <li key={key}>
                <button onClick={() => handleToggleColor(key)} className="w-full sm:flex gap-3 items-center mb-4 border border-zinc-500 p-4">
                  <h3 className="sm:text-xl font-bold text-center">{key}</h3>
                  <div className="flex flex-col items-center gap-2 w-full">
                    <div className="sm:w-[55%] h-auto rounded-full overflow-hidden">
                      <img src={`images/${yarn.image}`} />
                    </div>
                    <div className="text-xs sm:text-sm uppercase font-light">{yarn.name}</div>
                  </div>
                </button>
              </li>
            ))}
